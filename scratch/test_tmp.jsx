...
export default function Purchase() {
  ...
  const handlePdfFileSelect = async (e) => {
    ...
    toast.info(`Parsing ${isPdf ? 'PDF Invoice' : 'Invoice Image'}...`)

    const reader = new FileReader()
    reader.onload = async (ev) => {
      const dataUrl = ev.target.result
      setImportImagePreview(isPdf ? null : dataUrl)

      try {
        const res = await fetch(`${API()}/api/admin/parse-invoice-pdf`, {
          method: 'POST',
        ...
        if (res.ok) {
          ...
          if (data.items && data.items.length > 0) {
            ...
            if (data.supplierName && !poForm.supplier) {
              setPoForm(prev => ({ ...prev, supplier: data.supplierName }))
            }
            setShowImageImportModal(true)
            toast.success(`Extracted ${data.items.length} items from ${file.name}!`)
          } else {
            const ocrItems = await processPOImageOCR(dataUrl)
            setImportedItems(ocrItems)
            if (ocrItems.length > 0) {
              setShowImageImportModal(true)
              toast.success(`Extracted ${ocrItems.length} items from ${file.name}`)
            } else {
              toast.error('Could not extract items from PDF. Try adding manually.')
            }
          }
        } else {
          toast.error('Failed to process PDF invoice.')
        }
      } catch (err) {
        console.error('PDF parsing error:', err)
        toast.error('PDF parsing error: ' + err.message)
      } finally {
        setIsProcessingImage(false)
      }
    }
