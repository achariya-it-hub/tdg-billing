...
class ApiService {
  ...
  Future<Map<String, dynamic>> initiateCashfreePayment({
    required double amount,
    String? orderId,
    String? customerName,
    String? customerPhone,
    ...
    final url = Uri.parse('${AppConfig.baseUrl}/cashfree/create-order');
    try {
      final response = await http.post(
        url,
        headers: _getHeaders(),
      ...
      final data = jsonDecode(response.body);
      if (response.statusCode == 200 && data['success'] == true) return data;
      throw Exception(data['error'] ?? 'Failed to initiate Cashfree payment');
    } catch (e) {
      _handleError(e);
    }
  }
